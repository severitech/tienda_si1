<div>
    <div class="w-full p-4 bg-white shadow-xl sm:p-6 rounded-xl dark:bg-zinc-900">
        ...
        <div class="flex flex-col gap-3 mb-6 sm:flex-row sm:items-center sm:justify-between">
            <h1 class="text-2xl font-bold text-center text-gray-800 sm:text-3xl dark:text-white sm:text-left">Reporte de Ventas</h1>
        ...
        <div class="p-4 mb-6 bg-white rounded-lg shadow-lg dark:bg-zinc-800">
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5">
                ...
                <div>
                    <label class="block mb-1 text-sm font-medium text-gray-700 dark:text-gray-200">Cantidad Mínima</label>
                ...
                <div>
                    <label class="block mb-1 text-sm font-medium text-gray-700 dark:text-gray-200">Cantidad Máxima</label>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200 dark:bg-zinc-900 dark:divide-zinc-700">
                        @forelse($ventas as $venta)
                            <tr class="hover:bg-gray-50 dark:hover:bg-zinc-800">
                                <td class="px-4 py-4 whitespace-nowrap">{{ \Carbon\Carbon::parse($venta->fecha_venta)->format('d/m/Y H:i') }}</td>
                                <td class="px-4 py-4 whitespace-nowrap">#{{ $venta->venta_id }}</td>
                                <td class="px-4 py-4 whitespace-nowrap">
                                    <div>
                                        <div class="font-medium">{{ $venta->codigo_producto }}</div>
                                        <div class="text-sm text-gray-500 dark:text-gray-400">{{ $venta->nombre_producto }}</div>
                                    </div>
                                </td>
                                <td class="px-4 py-4 whitespace-nowrap">Bs.{{ number_format($venta->precio_unitario, 2) }}</td>
                                <td class="px-4 py-4 whitespace-nowrap">
                                ...
                                <td class="px-4 py-4 font-medium whitespace-nowrap">Bs.{{ number_format($venta->subtotal, 2) }}</td>
                                <td class="px-4 py-4 whitespace-nowrap">{{ $venta->nombre_usuario ?? 'N/A' }}</td>
                                <td class="px-4 py-4 whitespace-nowrap">{{ $venta->nombre_cliente ?? 'N/A' }}</td>
                            </tr>
                        @empty
                            <tr>
        ...
        @if (count($ventas) > 0)
            <div class="flex justify-between mt-4 text-sm text-gray-600 dark:text-gray-300">
                <span>Total: <strong>Bs.{{ number_format($total_ventas, 2) }}</strong></span>
            </div>
        @endif
    </div>
</div>
